...
function getGenAIClient(customKey?: string) {
  ...
  if (!apiKey) {
    throw new Error('GEMINI_API_KEY is missing in process environment and no custom key provided.');
  }
...
const geminiScheduler = new GeminiRateLimitedQueue(5000);
    ...
    try {
      ...
      if (isTransient && attempt < maxRetries) {
        ...
        if (isRateLimit) {
          ...
          // Respect server retryDelay (e.g. 40s) + 2.5s safety buffer, default to 40s + buffer if unparsed
          delay = (serverDelayMs ? serverDelayMs : 40000) + 2500 + Math.floor(Math.random() * 1500);
          console.warn(`[Gemini 429 Quota Delay] Server requested retryDelay. Waiting ${Math.round(delay / 1000)}s for Free Tier quota recovery before attempt ${attempt + 1}/${maxRetries}...`);
        } else {
          // Exponential backoff for 500/503/parse errors/network: 2s, 4s, 8s, 16s + jitter
...
// Phase 2: Generate Scene Directions via Gemini
app.post('/api/generate-directions', async (req, res) => {
  try {
    const { productionJson, scenePlansBatch, modelName = 'gemini-3.6-flash', customApiKey, creatorFacts = [] } = req.body;

    if (!productionJson || !scenePlansBatch || !Array.isArray(scenePlansBatch)) {
      return res.status(400).json({ error: 'Missing productionJson or scenePlansBatch array.' });
    }
      ...
      properties: {
        scenes: {
          ...
          items: {
            ...
            required: [
              'number', 'subject', 'product_visual_state', 'primary_action',
              'supporting_motion', 'environment_description', 'camera',
              'lighting_and_material', 'continuity_from_previous', 'transition_to_next',
              'required_visible_features', 'forbidden_elements', 'temporal_action'
            ]
          }
        }
      },
...
Return a structured JSON object containing a "scenes" array matching these ${currentBatchPlans.length} scenes in exact sequence.`;
      ...
      try {
        ...
        const { parsed } = await generateContentWithRetry(
          ...
          4, // Single authority: 4 attempts max per batch
          'scenes'
        );

        // Ensure original scene numbers are accurately attached if missing/mismatched
      ...
      if (!batchSuccess) {
        // Return partial results so completed batches are preserved
    ...
    // Sort merged scenes by scene number
    allScenes.sort((a, b) => (a.number || 0) - (b.number || 0));

    res.json({ success: true, scenes: allScenes });
  } catch (err: any) {
...
// Phase 3: Generate T2V Prompts via Gemini
app.post('/api/generate-prompts', async (req, res) => {
  try {
    ...
    if (!sceneDirectionsBatch || !Array.isArray(sceneDirectionsBatch)) {
      return res.status(400).json({ error: 'Missing sceneDirectionsBatch array.' });
    }

    const ai = getGenAIClient(customApiKey);

    const profileRules = t2vProfile === 'OMNI_FLASH'
      ...
      properties: {
        prompts: {
          ...
          items: {
            ...
            required: [
              'number', 'start', 'end', 'duration', 'stage_id', 'state',
              'action_description', 'voiceover', 'video_prompt',
              'stock_keywords', 'continuity_notes', 'quality_flags'
            ]
          }
        }
      },
...
Return a structured JSON object containing a "prompts" array in exact sequence for these ${currentDirectionsBatch.length} scenes.`;
      ...
      try {
        ...
        // Ensure original scene numbers/timestamps/voiceovers are attached if missing
    ...
    // Sort merged prompts by scene number
    allPrompts.sort((a, b) => (a.number || 0) - (b.number || 0));
